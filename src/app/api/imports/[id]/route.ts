...
// Descarta o lote pendente (e suas transações).
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const batchId = toNumber(id)
    if (!batchId) return err('ID da importação inválido.', 400)
    const result = await ImportService.discardBatch(batchId, user.id)
    return ok(result, result.message)
  } catch (error) {
    return apiError(error, 'Erro ao descartar importação.')
  }
}
