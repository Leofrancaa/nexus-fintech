...
export async function GET(request: NextRequest) {
  try {
    ...
    const atual = await IncomeService.getIncomeStats(user.id, mes, ano, catId)
    const mesAnterior = mes === 1 ? 12 : mes - 1
    const anoAnterior = mes === 1 ? ano - 1 : ano
    const anterior = await IncomeService.getIncomeStats(user.id, mesAnterior, anoAnterior, catId)
    ...
    return ok(stats, 'Estatísticas de receitas recuperadas com sucesso.')
  } catch (error) {
    return apiError(error, 'Erro ao buscar estatísticas de receitas.')
  }
}
