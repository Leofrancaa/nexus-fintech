...
export async function GET(request: NextRequest) {
  try {
    ...
    const resumo = await ExpenseService.getExpensesByCategory(user.id, month, year)
    return ok(resumo, 'Resumo por categorias recuperado com sucesso.')
  } catch (error) {
    return apiError(error, 'Erro ao buscar resumo por categorias.')
  }
}
